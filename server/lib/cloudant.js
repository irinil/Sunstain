 ...
 */
function create(type, name, description, quantity, location, contact, price, userID) {
    return new Promise((resolve, reject) => {
        let itemId = uuidv4();
 ...
 * when the call to the DB completes
 */
function update(id, type, name, description, quantity, location, contact, price, userID) {
    return new Promise((resolve, reject) => {
        db.get(id, (err, document) => {
            if (err) {
                ...
                if (price) {item["price"] = price} else {item["price"] = document.price};
